...
export interface PgpSignature2021Options {
  key?: PgpKeyPair2021;
  date?: any;
  signer?: any;
}

export class PgpSignature2021 {
  public useNativeCanonize: boolean = false;
  public key?: PgpKeyPair2021;
  public proof: any;
  public date: any;
  public creator: any;
  public type: string = 'https://w3id.org/security#PgpSignature2021';
  public signer: any;
  public verifier: any;
  public verificationMethod?: string;
  public getVM: any;
  constructor(options: PgpSignature2021Options = {}) {
  ...
  async verifySignature({ verifyData, verificationMethod, proof }: any) {
    ...
    if (!verifier) {
      ...
      if (verificationMethod['sec:publicKeyPgp']) {
        console.warn(
          'The current security context used by JSON-LD Signatures does not understand publicKeyPgp.'
        );
        verificationMethod.publicKeyPgp =
          verificationMethod['sec:publicKeyPgp'];
        delete verificationMethod['sec:publicKeyPgp'];
      }
      ...
      const key = await PgpKeyPair2021.from(verificationMethod);

      verifier = key.verifier();
    }
    return verifier.verify({ data: verifyData, signature: proof.proofValue });
  }
